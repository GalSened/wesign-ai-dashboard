...
async function testActualResponse() {
  ...
  try {
    ...
    // Wait for assistant message container to appear (with typing indicator)
    console.log('⏳ Waiting for typing indicator...');
    await page.waitForSelector('.message.assistant', { timeout: 10000 });

    // Wait for typing indicator to disappear (response is done)
    console.log('⏳ Waiting for typing indicator to disappear...');
    await page.waitForSelector('.typing-indicator:not(.active)', { timeout: 60000 });
    console.log('✅ Typing indicator gone, response should be complete');
    ...
    for (let i = 0; i < allMessages.length; i++) {
      ...
      console.log(`Message ${i + 1} - innerText (length: ${innerText.length}):`);
      console.log(innerText.substring(0, 500));
      console.log(`\nMessage ${i + 1} - innerHTML (first 500 chars):`);
